...
> 本文档是流水线「诊断」阶段的单一事实源，由 `/stk` 主入口（状态机推进至此阶段时）或高级单步命令 `/stk-diagnose` 按需读取执行。
...
3. 读取 `./save-token/diagnosis-report.md`，在对话中如实展示诊断报告（含流水线状态图所需的各分类 Token 占用）。

## 注意

- 如实展示 `diagnosis-report.md` 的内容即可，无需点评或者给出建议。
- 展示完毕即本阶段结束：由调用方（`/stk` 停点确认或用户手动发起下一命令）决定是否续跑「分析」阶段，本文档不主动推进。
