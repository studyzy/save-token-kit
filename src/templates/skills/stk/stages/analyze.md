...
> 本文档是流水线「分析」阶段的单一事实源，由 `/stk` 主入口（状态机推进至此阶段时）或高级单步命令 `/stk-analyze` 按需读取执行。子 Agent 详细规则见本 skill 目录下 `../agents/`。
...
**步骤 1.5: 收集使用频率数据（仅 CodeBuddy）**

检查 `./save-token/usage-report.json`：存在且 `scannedAt` 距当前 ≤ 24 小时 → 复用，跳到步骤 2。否则运行：

```bash
stk usage
```

（秒级本地磁盘扫描，统计会话历史中 Tool / Skill / SubAgent / MCP 的真实调用次数，落盘 `save-token/usage-report.json`。）

- **平台限制**：仅当诊断报告 `agentName === 'codebuddy'` 时执行（会话历史格式仅 CodeBuddy 已支持）；其他平台跳过本步骤，子 Agent 的"30 天未使用"数据规则相应跳过，**不视为阻断**。
- **运行失败或 `filesScanned === 0`** → 照常继续分析，但子 Agent **不得**将"无数据"充当"未使用"证据。
- 该报告是三类**使用数据驱动**建议的最终依据：`skill-opt`（Skill 30 天未用 → 移除）、`mcp-opt`（MCP 30 天未用 → 禁用）、`agent-opt`（SubAgent 30 天未用 → 移除），详见各 Agent 文档。
...
**第一轮（必问）— 使用场景与用户角色：3 个问题 MUST 合并为一次 `AskUserQuestion` 调用**（该工具单次最多支持 4 问，禁止拆成 3 次连续打断用户）：
...
在**单条消息**中并行发起所有需启动的子 Agent（多次 `Agent` 调用）。每个子 Agent 接收：诊断报告相关字段 + `context.json` + `repo-scan.json`（按需）+ `usage-report.json`（`skill-opt` / `mcp-opt` / `agent-opt` 必读，其余 Agent 忽略），并**自行落盘**统一 Schema JSON 到 `save-token/suggestions-<agent-name>.json`。
...
**⚠️ 必须提醒用户**：`save-token/tasks.md` 中的每条建议均需用户**仔细核对**。若某条不想执行，用户可直接在 `tasks.md` 中删除该行（或在执行优化前手动移除）。所有优化均为用户侧配置变更，工具不自动执行——执行哪些任务由用户在「优化」阶段的任务级选择中显式确认（高级单步用户则为 `/stk-optimize` 的任务选择环节）。
...
- 各子 Agent 详细规则见本 skill 目录下 `../agents/` 对应文件，按需读取。
- 展示摘要与 tasks.md 后本阶段结束：是否续跑「优化」阶段由调用方（`/stk` 停点确认）或用户决定，本文档不主动推进。
