...
> 本文档是流水线「优化」阶段的单一事实源，由 `/stk` 主入口（状态机推进至此阶段时）或高级单步命令 `/stk-optimize` 按需读取执行。

本阶段读取分级待办清单，**展示编号清单并由用户通过任务级选择圈定执行集合**，按 operationType 路由到对应子 Agent 逐项落地优化。

## 前置条件

- `./save-token/tasks.md` 存在（由「分析」阶段生成）。
- 若缺失：提示用户先运行 `/stk-analyze`（或 `/stk` 从分析阶段续跑），停止。不要凭空生成任务。
...
- 复选框：`- [ ]`（未完成）/ `- [x]`（已完成，可跳过）。
- 编号：`- [ ]` 与等级标签之间的 `<N.M>`（如 `1.1`），为用户可见的全局唯一编号，可用于向用户引用/指定任务。
- 等级标签：`[初级]` / `[中级]` / `[高级]`，位于编号之后。
- 动作描述：等级标签后的文本（如"禁用 skill: ponytail-help"、"启用 Headroom"）。
- 原因：缩进两空格的 `原因：` 行。
- no-op 识别：描述以"保持/保留/当前配置"开头且预估节省为 0 → 标记 `skipped`，不执行任何修改。

### 阶段 2: 展示清单并让用户选择执行哪些任务（任务级选择）

解析完成后，先在对话中展示完整任务清单（编号 + 等级 + 动作描述 + 预估节省），随后 **MUST** 用 `AskUserQuestion` 询问「本次执行哪些任务？」：

- 选项（单选）：
  1. **全部（推荐）**
  2. **仅初级**
  3. **初级 + 中级**
- 自由输入：用户可在 Other 中直接输入编号清单圈定任意子集（如 `1.1, 2.3, 3.1`）。

**硬性约束**：用户未显式回答前，**不得执行任何任务、不得修改任何文件**。默认推荐项仅为降低操作成本，绝不代替确认。
...
- 仅初级 → 所有 `[初级]` 任务
- 初级 + 中级 → 所有 `[初级]` + `[中级]` 任务
- 编号清单 → 精确匹配编号的任务（编号不存在时提示并请用户更正，不猜测）

已勾选（`- [x]`）任务不进入执行队列；用户圈定集合外的任务保持 `- [ ]` 不变（留待下次执行）。no-op 任务不进入执行队列（落盘 `skipped`）。
...
> 本阶段操作对象均为当前 Git 仓库内的受版本管理文件，**无需手动备份，直接修改即可**——误改可由 `git checkout` / `git restore` 还原。仅当任务明确指向仓库外用户配置（如 `~/.codebuddy/settings.json`、`.mcp.json` 等脱离 Git 跟踪的文件）时才先备份再改。
