...
- 主文件每次会话全量注入上下文。CodeBuddy 官方文档建议主文件作**索引/资源地图**：项目描述 + 常用命令 + 关键文件指针；细节下沉到 `@引用` 文件或 Skills，**按需加载**。
...
- 本 agent 可建议"下沉为 @引用 文档（如 `@docs/xxx.md`）"或"下沉为 rules"，但**绝不产出"下沉为 skill"建议**——skill 是给 agent 执行的流程/能力，不是文档容器，CODEBUDDY.md 的章节内容无法承载为 skill
...
| 含按需加载机会 | 项目级细节（如文档读取约定、特定规则）未拆为 `.codebuddy/rules/*.md` 或 `@引用` 文档 | **本 agent 仅建议"下沉为 @引用 文档 或 rules"**，**不产出"拆分为 skill"建议**（skill 非文档容器）。`action`: "将 CODEBUDDY.md <章节名> 下沉为 @docs/xxx.md 或 rules"，`reason`: "细节按需加载降低常驻占用" |
