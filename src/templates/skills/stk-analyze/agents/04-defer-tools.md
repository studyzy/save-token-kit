...
- `builtinTools`：每项含 `name` / `estimatedTokens` / `category`（`builtin` / `mcp` / `deferred`）
...
**工具名来源**：从 `builtinTools` 按 `category` 过滤获取真实挂载工具名。
...
- `builtinTools` 缺失导致无法查得真实工具名 → `skipped: true`
...
- **部分 Defer**：`deferred_tools_count × perToolTokens`，perToolTokens 取 builtinTools 值或 80 兜底
